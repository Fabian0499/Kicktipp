...
export function ProfileAvatarUpload({
  ...
  return (
    <article className="rounded-xl border bg-white p-5 text-zinc-900 shadow-sm">
      <h2 className="text-sm font-medium text-zinc-600">{t("profile.title")}</h2>
      <div className="mt-3 flex items-center gap-4">
        {avatarUrl ? (
          <Image
            src={avatarUrl}
            alt={`${t("leaderboard.profileAlt")} ${name}`}
            width={64}
            height={64}
      ...
      <p className="mt-2 text-xs text-zinc-500">{t("profile.allowedFormats")}</p>
